...
- **現状の注意点**: 記事はまだ非公開(公開時刻未定、少なくとも本ページ作成時点では未公開)のため、CTAボタンの `href` は暫定的に `#` のプレースホルダーのままにしてある。**記事のURLが判明次第、2箇所の `<a class="btn en">` の `href` を実際のnote記事URLに差し替えること。**
...
- CTAボタンの `href="#"` は記事公開後に必ず実URLへ差し替える(上記「現状の注意点」参照)
