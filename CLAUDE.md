...
- **紹介する記事**: 「月3〜4件しか予約が入らなかった私が、集客を変えた方法」(note有料記事)
- **ヒーローの見出し**: 「自宅サロンで『予約が来ない』を抜け出した。月商5桁から6桁へ変わるまでにやったこと」に変更済み(記事本文・WHYセクションは「月3〜4件」表記のまま残っているため、内容の整合を取りたい場合はそちらの見直しも検討する)
- **導線**: ページ内の「記事を読む →」ボタン(ヒーロー・CTAバナーの2箇所)からnoteの記事ページへ
- **公開時刻ゲート**: 記事は2026年9月6日21:00に公開予定。それまでCTAボタン(`.note-cta` でラップした「記事を読む →」+その下の注記、ヒーロー・CTAバナーの2箇所)は非表示にし、公開時刻を過ぎたら自動的に表示するよう `<script>` 内でハードコードした日時(`new Date(2026, 8, 6, 21, 0, 0, 0)`)と比較して切り替えている。ページを開いたまま公開時刻をまたいだ場合もリロード無しで表示される。リンク先は `https://note.com/preview/n50b71d4197ac?prev_access_key=1498209ad74e993c982816ca4fb90708`(note側のプレビュー用URLのため、正式公開後に本URLが変わる場合は差し替えが必要)

## 構成(セクション順)

ヘッダー(ロゴのみ)→ 導入フレームアニメーション → ヒーロー(記事タイトル+CTA)→ ギャラリー(スクラップブック風ランダム配置)→ なぜこの記事を書いたか → 作り手の想い(手元写真付き)→ CTAバナー → フッター
...
- ヒーローは背景を5枚(タイトルバナー画像・動画・写真3枚)でクロスフェードするスライドショーにし、下部をアイボリーへ滑らかにグラデーションさせて本文セクションへ繋げている
...
- **ヒーローのスライドショー(`.hero__slide`)**: `.hero__bg` 内に5枚(①タイトル文字入りバナー画像 ②動画(`hero-video.mp4`、muted/loop/playsinline、音声トラックは`ffmpeg`で除去済み) ③店先の写真 ④悩む女性の写真 ⑤ベッドで横たわる写真)を重ねて配置し、4.5秒ごとに`.is-active`クラスをJSで付け替えてopacityのtransition(1.8s)でクロスフェードする。1枚目(タイトルバナー)は画像自体に見出し文字が入っているため、表示中は`.hero.is-title-slide`クラスでHTML側のeyebrow/h1/hero__leadを`opacity:0`にして二重表示を防いでいる(スライドが1枚目に戻るたびJSで付け直す)。`prefers-reduced-motion`時は自動切り替えを行わず1枚目で静止させる
...
| 用途 | ファイル |
|---|---|
| ヒーロー背景・旧フラットレイ(現在は`.hero__slide`の一枚目には未使用、他用途にも参照なし) | `assets/hero-flatlay.jpg` |
| ヒーロースライドショー(5枚) | `assets/hero-slide-title.jpg`(タイトルバナー)/ `hero-video.mp4`(動画)/ `hero-slide-storefront.jpg`(店先)/ `hero-slide-worry.jpg`(悩む女性)/ `hero-slide-bed.jpg`(ベッドで横たわる女性) |
...
`hero-slide-storefront.jpg`/`hero-slide-worry.jpg` は、ギャラリーの`gallery-01.jpg`/`gallery-02.jpg`と同じ元写真だが、ギャラリー側は正方形にトリミング済みなのに対し、ヒーロー側は元のワイドな比率のまま(フルブリードの背景として使うため)別ファイルとして保存している。

**注意**: いただいた元画像(`hero-flatlay.jpg` の元データ)は上下に白い余白(レターボックス)が入っていたため、そのまま `object-fit: cover` で表示すると白帯が写り込んでしまう。素材化する際は余白部分を事前にトリミングしてから保存すること(このページでは実際にその不具合が起きたため、白でない部分の上下端を検出してクロップし直して解決した)。

## 変更してはいけない部分 / 注意点

- 本体LP(KOTONOHA)や `works/` 配下の各サンプルLPの変更にこのページを巻き込まない(独立した制作物として扱う)
- CTAボタンの公開時刻ゲート(`<script>` 内の `new Date(2026, 8, 6, 21, 0, 0, 0)`)とnoteのリンク先URLは、上記「現状の注意点」の内容と齟齬が出ないよう、記事の公開状況が変わった場合は必ず更新する
